...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(char => char).map {
    case (char: Char, list: List[Char]) => (char, list.size)
  }.toList.sortBy(_._1)


  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    val senAsWord = s.foldLeft("")((acc, w) => acc + w)
    wordOccurrences(senAsWord)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def occurrenceAnagram(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
      else {
        for (
          subset <- combinations(occurrences) if dictionaryByOccurrences.contains(subset);
          word <- dictionaryByOccurrences(subset);
          otherWords <- occurrenceAnagram(subtract(occurrences, subset))
        )
